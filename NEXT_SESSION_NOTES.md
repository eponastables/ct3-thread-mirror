...
**Command status:** CT3 is the operative CT-series agent as of the formal activation acknowledgement in Ledger commit `292d5b9`.

At the next session start, first read the current unified Ledger head, CT3 mailbox, CT3 Initial Command State, the current TAT2 calibration record, and any value-free update on the legacy credential-exposure notice. Confirm `main` and `master` remain identical before issuing any material governance action.
...
CT3 has issued the final **limited operational dashboard release** in Ledger commit `945416a`, applying only to checkpoint `manus-webdev://076ef52f`. The dashboard is now the operative reference for normal recordkeeping, review, non-destructive VOID, and complete/performance export. `PACE_ADJUSTMENT_ENABLED` remains disabled. TAT2 may not use pace-adjusted metrics for price construction or calibration conclusions. The first post-release integrity review is due after five new operational records, or immediately after any mutation, VOID, export, or audit anomaly.
